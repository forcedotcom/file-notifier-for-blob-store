...
function is_valid_consumer_key_name {
    ...
    if [ -z "$CONSUMER_KEY_NAME" ]; then
      add_validation_error "Error: CONSUMER KEY name is missing/empty. Please provide a valid CONSUMER KEY name."
    elif [[ "$CONSUMER_KEY_NAME" =~ $regex ]]; then
        echo "consumer key ${CONSUMER_KEY_NAME} is valid"
        echo "consumer key ${CONSUMER_KEY_NAME} is valid" >> $log_filename
    else
        add_validation_error "Error: Invalid Azure CONSUMER KEY Name: ${CONSUMER_KEY_NAME}, Consumer Key name must contain only alphanumeric characters, hyphens, and underscores."
    fi
}
...
function is_valid_rsa_private_key_name {
    ...
    if [ -z "$RSA_PRIVATE_KEY_NAME" ]; then
      add_validation_error "Error: RSA PRIVATE KEY name is missing/empty. Please provide a valid RSA PRIVATE KEY name."
    elif [[ "$RSA_PRIVATE_KEY_NAME" =~ $regex ]]; then
        echo "rsa private key name ${RSA_PRIVATE_KEY_NAME} is valid"
        echo "rsa private key name ${RSA_PRIVATE_KEY_NAME} is valid" >> $log_filename
    else
        add_validation_error "Error: Invalid RSA PRIVATE KEY Name: ${RSA_PRIVATE_KEY_NAME}, RSA PRIVATE KEY name must contain only alphanumeric characters, hyphens, and underscores."
    fi
}
...
if az keyvault key show --vault-name $KEY_VAULT_NAME --name $RSA_PRIVATE_KEY_NAME 2>&1 | grep -q "KeyNotFound"; then
  az keyvault key import --vault-name $KEY_VAULT_NAME --name ${RSA_PRIVATE_KEY_NAME} --pem-file $PEM_FILE_PATH --protection software >> $log_filename
  if [ $? -eq 0 ]; then
    echo "Step 9/14 : Successfully created secret with name ${RSA_PRIVATE_KEY_NAME} under KeyVault ${KEY_VAULT_NAME}"
    echo "Step 9/14 : Successfully created secret with name ${RSA_PRIVATE_KEY_NAME} under KeyVault ${KEY_VAULT_NAME}" >> $log_filename 
  else
    echo "Step 9/14 : Failed to create secret with name ${RSA_PRIVATE_KEY_NAME} under KeyVault ${KEY_VAULT_NAME}"
    echo "Step 9/14 : Failed to create secret with name ${RSA_PRIVATE_KEY_NAME} under KeyVault ${KEY_VAULT_NAME}" >> $log_filename
    exit
  fi
else 
  echo "Step 9/14 : Key with name ${RSA_PRIVATE_KEY_NAME} exists under KeyVault ${KEY_VAULT_NAME}, skipping the creation of ${RSA_PRIVATE_KEY_NAME}";
  echo "Step 9/14 : Key with name ${RSA_PRIVATE_KEY_NAME} exists under KeyVault ${KEY_VAULT_NAME}, skipping the creation of ${RSA_PRIVATE_KEY_NAME}" >> $log_filename
fi

if az keyvault secret show --vault-name $KEY_VAULT_NAME --name $CONSUMER_KEY_NAME 2>&1 | grep -q "SecretNotFound"; then
  az keyvault secret set --vault-name $KEY_VAULT_NAME --name $CONSUMER_KEY_NAME --value $CONSUMER_KEY_VALUE >> $log_filename
  if [ $? -eq 0 ]; then
    echo "Step 10/14 : Successfully created key with name ${CONSUMER_KEY_NAME} under ${KEY_VAULT_NAME}"
    echo "Step 10/14 : Successfully created key with name ${CONSUMER_KEY_NAME} under ${KEY_VAULT_NAME}" >> $log_filename 
  else
    echo "Step 10/14 : Failed to create key with name ${CONSUMER_KEY_NAME} under ${KEY_VAULT_NAME}"
    echo "Step 10/14 : Failed to create key with name ${CONSUMER_KEY_NAME} under ${KEY_VAULT_NAME}" >> $log_filename 
    exit
  fi
else
  echo "Step 10/14 : Secret with name ${CONSUMER_KEY_NAME} exists under KeyVault ${KEY_VAULT_NAME}, skipping the creation of ${CONSUMER_KEY_NAME}";
  echo "Step 10/14 : Secret with name ${CONSUMER_KEY_NAME} exists under KeyVault ${KEY_VAULT_NAME}, skipping the creation of ${CONSUMER_KEY_NAME}" >> $log_filename 
fi
