...
while true; do
  echo "As a pre-requiste you should have secretmanager access to ${PROJECT_NUMBER} project, do you have? (yes/no) : "
  read user_input

  if [ "$user_input" == "yes" ]; then
    break
  elif [ "$user_input" == "no" ]; then
    echo "IMPORTANT NOTE : As a pre-requiste you should have secretmanager access to ${PROJECT_NUMBER} project, please get the secretmanager access and then try again"
    exit
  else
    echo "Invalid input. Please enter 'yes' or 'no'."
  fi
...
if [ ${#validation_errors[@]} -gt 0 ]; then
  ...
  echo "NOTE: Please check sample config file at https://git.soma.salesforce.com/rdesai/unstructuredscripts/blob/master/input_parameters_gcs_conf_description.txt"
  echo "NOTE: Please check sample config file at https://git.soma.salesforce.com/rdesai/unstructuredscripts/blob/master/input_parameters_gcs_conf_description.txt" >> $log_filename
  exit
else
...
gcloud functions deploy "${EVENT_GCS_BUCKET_SOURCE}-create-event-function" \
--gen2 \
--runtime=python311 \
--region=$GCS_REGION \
...
echo "Step 9/11 : Successfully deployed ${EVENT_GCS_BUCKET_SOURCE}-create-event-function"
echo "Step 9/11 : Successfully deployed ${EVENT_GCS_BUCKET_SOURCE}-create-event-function" >> $log_filename

gcloud functions deploy "${EVENT_GCS_BUCKET_SOURCE}-delete-event-function" \
--gen2 \
--runtime=python311 \
--region=$GCS_REGION \
--source=gs://$SOURCE_CODE_BUCKET_NAME/$FILE_NAME \
...
echo "Step 10/11 : Successfully deployed ${EVENT_GCS_BUCKET_SOURCE}-delete-event-function"
echo "Step 10/11 : Successfully deployed ${EVENT_GCS_BUCKET_SOURCE}-delete-event-function" >> $log_filename

gcloud functions deploy "${EVENT_GCS_BUCKET_SOURCE}-archive-event-function" \
--gen2 \
--runtime=python311 \
--region=$GCS_REGION \
--source=gs://$SOURCE_CODE_BUCKET_NAME/$FILE_NAME \
...
echo "Step 11/11 : Successfully deployed ${EVENT_GCS_BUCKET_SOURCE}-archive-event-function"
echo "Step 11/11 : Successfully deployed ${EVENT_GCS_BUCKET_SOURCE}-archive-event-function" >> $log_filename
echo "All the gcs cloud function installer logs are logged to ${log_filename} file"